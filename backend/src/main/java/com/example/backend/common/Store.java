...
public class Store {
  ...
  public void audit(UUID actor, String action, Object resource) {
    update(
        "INSERT INTO audit_log(actor_id,action,resource_id) VALUES(?,?,?)",
        actor,
        action,
        resource.toString());
  }

  public void event(UUID aggregate, String type) {
    update("INSERT INTO outbox_event(aggregate_id,type) VALUES(?,?)", aggregate, type);
  }

  public void alert(String type, Object id, String message) {
    update(
        "INSERT INTO alert(type,resource_id,message) VALUES(?,?,?) ON CONFLICT(type,resource_id) WHERE NOT resolved DO NOTHING",
        type,
        id.toString(),
        message);
  }
